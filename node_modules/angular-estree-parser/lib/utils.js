...
function parseNgBinding(input) {
    return parseNg(input, (astInput, ngParser) => ngParser.parseBinding(astInput, NG_PARSE_FAKE_LOCATION));
}
exports.parseNgBinding = parseNgBinding;
function parseNgSimpleBinding(input) {
    return parseNg(input, (astInput, ngParser) => ngParser.parseSimpleBinding(astInput, NG_PARSE_FAKE_LOCATION));
}
exports.parseNgSimpleBinding = parseNgSimpleBinding;
function parseNgAction(input) {
    return parseNg(input, (astInput, ngParser) => ngParser.parseAction(astInput, NG_PARSE_FAKE_LOCATION));
}
exports.parseNgAction = parseNgAction;
function parseNgTemplateBindings(input) {
    const ngParser = createNgParser();
    const { templateBindings: ast, errors } = ngParser.parseTemplateBindings(NG_PARSE_TEMPLATE_BINDINGS_FAKE_PREFIX, input, NG_PARSE_FAKE_LOCATION);
    assertAstErrors(errors);
    return ast;
}
...
function parseNgInterpolation(input) {
    ...
    const prefix = '{{';
    const suffix = '}}';
    const { ast: rawAst, errors } = ngParser.parseInterpolation(prefix + astInput + suffix, NG_PARSE_FAKE_LOCATION);
    assertAstErrors(errors);
