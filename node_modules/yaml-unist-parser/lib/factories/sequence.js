"use strict";
exports.__esModule = true;
var tslib_1 = require("tslib");
var end_comment_attachable_1 = require("./end-comment-attachable");
var leading_comment_attachable_1 = require("./leading-comment-attachable");
var node_1 = require("./node");
function createSequence(position, content, children) {
    return tslib_1.__assign({}, node_1.createNode("sequence", position), leading_comment_attachable_1.createLeadingCommentAttachable(), end_comment_attachable_1.createEndCommentAttachable(), content, { children: children });
}
exports.createSequence = createSequence;
